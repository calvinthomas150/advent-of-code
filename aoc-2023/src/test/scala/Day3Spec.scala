...
class Day3Spec extends AnyFlatSpec with should.Matchers:
  ...
  "getPart" should "return the part position based on being somewhere in the number" in :
    val line = "...$.*.743456....."
    implicit val maxCoordinate: (Int, Int) = (0, 17)
    val partOfNumber = (0,8)
    Day3.getPart(line, partOfNumber) should be(PartPosition(0,7,12))
  ...
  "getPartsFromSymbol" should "return the PartPosition of all the parts that are adjacent to the symbol" in:
    ...
    implicit val maxCoordinate: (Int, Int) = (9,9)

    Day3.getPartsFromSymbol((5,5), lines) should be (Set(PartPosition(6, 2, 4)))
    Day3.getPartsFromSymbol((1,3), lines) should be (Set(PartPosition(0, 0, 2), PartPosition(2,2,3)))
    Day3.getPartsFromSymbol((4,3), lines) should be (Set(PartPosition(4, 0, 2)))
  ...
  "getAllParts" should "return all parts based on the list of symbols provided" in:
    ...
    implicit val maxCoordinates: (Int, Int) = (3,9)

    Day3.getAllParts(lines, symbols) should be(List(PartPosition(0,0,2), PartPosition(2,2,3), PartPosition(2,6,8)))
  ...
  "isPartAdjacentToSymbol" should "return true if the part is touching a symbol (including diagonals) or false otherwise" in:
    ...
    Day3.isPartAdjacentToSymbol(symbol, partPosition) should be(true)

    val symbol1 = (3,3)
    val partPosition1 = PartPosition(3, 0, 2)
    Day3.isPartAdjacentToSymbol(symbol1, partPosition1) should be(true)

    val symbol2 = (3, 3)
    val partPosition2 = PartPosition(2, 2, 4)
    Day3.isPartAdjacentToSymbol(symbol2, partPosition2) should be(true)

    val symbol3 = (3, 3)
    val partPosition3 = PartPosition(4, 2, 2)
    Day3.isPartAdjacentToSymbol(symbol3, partPosition3) should be(true)

    val symbol4 = (3, 3)
    val partPosition4 = PartPosition(4, 4, 4)
    Day3.isPartAdjacentToSymbol(symbol4, partPosition4) should be(true)

    val symbol5 = (3, 3)
    val partPosition5 = PartPosition(4, 5, 5)
    Day3.isPartAdjacentToSymbol(symbol5, partPosition5) should be(false)

    val symbol6 = (3, 3)
    val partPosition6 = PartPosition(3, 5, 5)
    Day3.isPartAdjacentToSymbol(symbol6, partPosition6) should be(false)
  ...
  "findGears" should "find the gears in the system, i.e. where the symbol is adjacent to exactly two part positions" in:
    ...
    Day3.findGears(symbols, parts) should be(gears)
